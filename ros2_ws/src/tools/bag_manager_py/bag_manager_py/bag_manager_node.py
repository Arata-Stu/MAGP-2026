...
class RosBagManagerNode(Node):
    ...
    def memo_callback(self, msg: String):
        ...
        if memo not in ["good", "bad"]:
            return
        if not self.last_record_dir or not os.path.exists(self.last_record_dir):
            return
        if self.is_recording:
            return
        self._apply_memo_flag(memo)

    def _apply_memo_flag(self, memo: str):
        src = self.last_record_dir.rstrip('/')
        dir_parent, dir_name = os.path.split(src)
        if dir_name.endswith('_good'):
            dir_name = dir_name[:-5]
        elif dir_name.endswith('_bad'):
            dir_name = dir_name[:-4]
        dst_name = f"{dir_name}_{memo}"
        dst = os.path.join(dir_parent, dst_name)
        if src == dst:
            return
        try:
            os.rename(src, dst)
            self.last_record_dir = dst
            self.get_logger().info(f"ディレクトリ名変更: {src} → {dst}")
        except Exception as e:
            self.get_logger().error(f"リネームに失敗しました: {e}")
